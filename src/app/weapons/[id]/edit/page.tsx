...
export default function WeaponEditPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const weaponData = {
        ...
        level_requirement: parseInt(formData.get('level_requirement') as string),
      ...
      await apiClient.patch(`/admin/weapons/${params.id}`, { weapon: weaponData })
      router.push(`/weapons/${params.id}`)
    } catch (error: any) {
      if (error.response?.data?.errors) {
        setErrors(error.response.data.errors)
      } else {
        setErrors(['武器の更新に失敗しました'])
      }
    } finally {
      setSaving(false)
    }
      ...
      <div className="p-6">
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-bold text-gray-900">武器編集: {weapon.name}</h1>
            <Link
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div>
                <label htmlFor="weapon_category" className="block text-sm font-medium text-gray-700">
                  武器カテゴリ *
                </label>
                <select
                  name="weapon_category"
                  id="weapon_category"
                  defaultValue={weapon.weapon_category}
                  required
                  className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                >
              ...
              <div>
                ...
                <input
                  ...
                  defaultValue={weapon.level_requirement}
                  min="1"
                  max="100"
                  className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                />
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  name="sell_price"
                  id="sell_price"
                  defaultValue={weapon.sell_price}
                  min="0"
                  className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  id="job_requirement"
                  defaultValue={weapon.job_requirement?.join(', ')}
                  placeholder="戦士, パラディン, 魔剣士"
                  className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
